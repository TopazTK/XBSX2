...
namespace VMManager
{
	...
	static void CheckForConfigChanges(const Xbsx2Config& old_config);
	static void CheckForCPUConfigChanges(const Xbsx2Config& old_config);
	static void CheckForGSConfigChanges(const Xbsx2Config& old_config);
	static void CheckForFramerateConfigChanges(const Xbsx2Config& old_config);
	static void CheckForPatchConfigChanges(const Xbsx2Config& old_config);
...
#define COMMON_DOWNLOAD_MESSAGE \
	"XBSX2 builds can be downloaded from https://xbsx2.net/downloads/"
	...
	{
		*error = "XBSX2 requires the Streaming SIMD 4 Extensions instruction set, which your CPU does not support.\n\n"
				 "SSE4 is now a minimum requirement for XBSX2. You should either upgrade your CPU, or use an older build such as 1.6.0.\n\n" COMMON_DOWNLOAD_MESSAGE;
		return false;
	}
	...
	{
		*error = "This build of XBSX2 requires the Advanced Vector Extensions 2 instruction set, which your CPU does not support.\n\n"
				 "You should download and run the SSE4 build of XBSX2 instead, or upgrade to a CPU that supports AVX2 to use this build.\n\n" COMMON_DOWNLOAD_MESSAGE;
		return false;
	}
#endif
#endif
...
void VMManager::LoadPatches(const std::string& serial, u32 crc, bool show_messages, bool show_messages_when_disabled)
{
	...
	if (show_messages)
	{
		...
		{
			message += " are active.";
			Host::AddIconOSDMessage("LoadPatches", ICON_FA_FILE_CODE, message, 3.0f);
		}
...
bool VMManager::DoSaveState(const char* filename, s32 slot_for_message, bool zip_on_thread)
{
	...
	{
		Host::AddIconOSDMessage(std::move(osd_key), ICON_FA_EXCLAMATION_TRIANGLE, fmt::format("Failed to save state: {}.", e.DiagMsg()), 5.0f);
		return false;
	}
}
	...
	const char* filename, s32 slot_for_message)
{
	...
	{
		Host::AddIconOSDMessage(std::move(osd_key), ICON_FA_EXCLAMATION_TRIANGLE, fmt::format("Failed to save state to slot {}.", slot_for_message), 5.0f);
	}
...
bool VMManager::ChangeDisc(CDVD_SourceType source, std::string path)
{
	...
	{
		Host::AddIconOSDMessage("ChangeDisc", ICON_FA_COMPACT_DISC, fmt::format("Failed to open new disc image '{}'. Reverting to old image.", display_name), 5.0f);
		CDVDsys_ChangeSource(old_type);
		if (!old_path.empty())
			CDVDsys_SetFile(old_type, std::move(old_path));
		if (!DoCDVDopen())
		{
			Host::AddIconOSDMessage("ChangeDisc", ICON_FA_COMPACT_DISC, "Failed to switch back to old disc image. Removing disc.", 5.0f);
			CDVDsys_ChangeSource(CDVD_SourceType::NoDisc);
			DoCDVDopen();
		}
	}
...
void VMManager::CheckForCPUConfigChanges(const Xbsx2Config& old_config)
{
...
void VMManager::CheckForGSConfigChanges(const Xbsx2Config& old_config)
{
...
void VMManager::CheckForFramerateConfigChanges(const Xbsx2Config& old_config)
{
...
void VMManager::CheckForPatchConfigChanges(const Xbsx2Config& old_config)
{
...
void VMManager::CheckForSPU2ConfigChanges(const Xbsx2Config& old_config)
{
...
void VMManager::CheckForDEV9ConfigChanges(const Xbsx2Config& old_config)
{
...
void VMManager::CheckForMemoryCardConfigChanges(const Xbsx2Config& old_config)
{
	bool changed = false;
...
void VMManager::CheckForConfigChanges(const Xbsx2Config& old_config)
{
	if (HasValidVM())
	{
...
void VMManager::WarnAboutUnsafeSettings()
{
	...
	if (!messages.empty())
	{
		if (messages.back() == '\n')
			messages.pop_back();
		Host::AddKeyedOSDMessage("unsafe_settings_warning", std::move(messages), 5.0f);
	}
	else
	{
	...
	if (!messages.empty())
	{
		if (messages.back() == '\n')
			messages.pop_back();
		Host::AddKeyedOSDMessage("performance_settings_warning", std::move(messages), 5.0f);
	}
	else
	{
...
static void HotkeyLoadStateSlot(s32 slot)
{
	if (s_game_crc == 0)
	{
		Host::AddIconOSDMessage("LoadStateFromSlot", ICON_FA_EXCLAMATION_TRIANGLE, "Cannot load state from a slot without a game running.", 5.0f);
		return;
	}
...
static void HotkeySaveStateSlot(s32 slot)
{
	if (s_game_crc == 0)
	{
		Host::AddIconOSDMessage("SaveStateToSlot", ICON_FA_EXCLAMATION_TRIANGLE, "Cannot save state to a slot without a game running.", 5.0f);
		return;
	}
...
#define DEFINE_HOTKEY_SAVESTATE_X(slotnum, slotnumstr) DEFINE_HOTKEY("SaveStateToSlot" #slotnum, \
	"4. Save States", "Save State To Slot " #slotnumstr, [](s32 pressed) { if (!pressed) HotkeySaveStateSlot(slotnum); })
...
#define DEFINE_HOTKEY_LOADSTATE_X(slotnum, slotnumstr) DEFINE_HOTKEY("LoadStateFromSlot" #slotnum, \
	"5. Load States", "Load State From Slot " #slotnumstr, [](s32 pressed) { \
		if (!pressed) \
